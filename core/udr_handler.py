...
async def udr_app_data_insert(traffic_influ_sub: TrafficInfluSub, intGroupID: str=None, supi: str=None, _id: str=None):
    ...
    if traffic_influ_sub.subscribed_events and "UP_PATH_CHANGE" in traffic_influ_sub.subscribed_events:
        ...
        traffic_influ_data.up_path_chg_notif_corre_id = _id
        traffic_influ_data.up_path_chg_notif_uri = f"http://{conf.HOSTS['NEF'][0]}/up_path_change/{_id}"
    ...
    conf.logger.info("--------------------udr influ data-------------------")
    conf.logger.info(traffic_influ_data)

    traffic_influ_sub.notification_destination = f"http://{conf.HOSTS['NEF'][0]}/up_path_change/{_id}"

    async with httpx.AsyncClient(http1=True if conf.CORE=="free5gc" else False, http2=None if conf.CORE=="free5gc" else True) as client:
            response = await client.put(
                f"http://{conf.HOSTS['UDR'][0]}/nudr-dr/v1/application-data/influenceData/{_id}",
