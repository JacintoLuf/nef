...
def create_sub():
    ...
    flow_info = FlowInfo(flow_id=1010,
                        flow_descriptions=["permit in ip from 10.200.0.4 to 10.255.32.123 80", "permit out ip from 10.255.32.123 80 to 10.200.0.4"])
...
def create_sub2():
    ...
    route_to_loc = RouteToLocation(dnai="internet", route_info=route_info)
    flow_info = FlowInfo(flow_id=1010,
                         flow_descriptions=["permit in ip from 10.200.0.4 to 10.255.32.132 80", "permit out ip from 10.255.32.132 80 to 10.200.0.4"])
