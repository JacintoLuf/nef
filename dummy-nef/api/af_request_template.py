...
def create_sub(ip):
    ...
    flow_info = FlowInfo(flow_id=1010,
                        flow_descriptions=[f"permit in ip from {ip} to 10.255.32.123", f"permit out ip from 10.255.32.123 to {ip}"])
    ...
    traffic_influ = TrafficInfluSub(
        ...
        ipv4_addr=ip,
        notification_destination=f"http://{conf.HOSTS['NEF'][0]}/pcf-policy-authorization-callback",
...
def create_sub3(ip):
    ...
    qos_sub = AsSessionWithQoSSubscription(
        ...
        notification_destination=f"http://{conf.HOSTS['NEF'][0]}/pcf-policy-authorization-qos-callback",
        flow_info=[flow_info],
        qos_reference="6",
